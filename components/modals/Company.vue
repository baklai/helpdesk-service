      ...
      <v-card-title class="pt-0">
        ...
        <v-menu offset-y open-on-hover>
          ...
          <v-list flat dense subheader>
            ...
            <v-list-item
              @click="onDelete()"
              v-if="$auth.hasScope('company:delete')"
            >
              <v-list-item-icon class="mr-1">
...
export default {
  ...
  computed: {
    title() {
      return this.$t('Сompany');
    },
    subtitle() {
      return this.$t('Companies from database');
    }
  },
  ...
  methods: {
    async onItem(id) {
      try {
        ...
        this.status = this.IDItem
          ? this.$t('Edit current record')
          : this.$t('Create new record');
        this.items = await this.$store.dispatch('api/company/findAll');
        if (this.IDItem) {
    ...
    async onSave() {
      if (this.$refs.form.validate()) {
        try {
          if (this.IDItem) {
            await this.$store.dispatch('api/company/updateOne', {
              id: this.IDItem,
              data: {
                ...this.item
              }
            ...
            await this.$store.dispatch('api/company/createOne', {
              ...this.item
            });
            await this.onUpdate();
    ...
    async onDelete() {
      try {
        if (this.IDItem) {
          await this.$store.dispatch('api/company/removeOne', {
            id: this.IDItem
          });
          await this.onUpdate();
