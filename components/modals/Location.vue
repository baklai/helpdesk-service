      ...
      <v-card-title class="pt-0">
        ...
        <v-menu offset-y open-on-hover>
          ...
          <v-list flat dense subheader>
            ...
            <v-list-item
              @click="onDelete()"
              v-if="$auth.hasScope('location:delete')"
            >
              <v-list-item-icon class="mr-1">
...
export default {
  ...
  computed: {
    ...
    subtitle() {
      return this.$t('Locations from database');
    }
  },
  ...
  methods: {
    async onItem(id) {
      try {
        ...
        this.status = this.IDItem
          ? this.$t('Edit current record')
          : this.$t('Create new record');
        this.items = await this.$store.dispatch('api/location/findAll');
        if (this.IDItem) {
          const item = await this.$store.dispatch(
            'api/location/findOne',
            this.IDItem
          );
          this.item = { ...item };
        } else {
    ...
    async onSave() {
      if (this.$refs.form.validate()) {
        try {
          if (this.IDItem) {
            await this.$store.dispatch('api/location/updateOne', {
              id: this.IDItem,
              data: {
                ...this.item
              }
            ...
            await this.$store.dispatch('api/location/createOne', {
              ...this.item
            });
            await this.onUpdate();
    ...
    async onDelete() {
      try {
        if (this.IDItem) {
          await this.$store.dispatch('api/location/removeOne', {
            id: this.IDItem
          });
          await this.onUpdate();
