      ...
      <v-card-title class="pt-0">
        <v-list flat>
          <v-list-item two-line class="pa-0">
            <v-list-item-avatar tile>
              <v-icon x-large> mdi-expansion-card-variant </v-icon>
            </v-list-item-avatar>
            <v-list-item-content>
              <v-list-item-title>
                {{ title }}
        ...
        <v-menu offset-y open-on-hover>
          ...
          <v-list flat dense subheader>
            ...
            <v-list-item
              @click="onDelete()"
              v-if="$auth.hasScope('unit:delete')"
            >
              <v-list-item-icon class="mr-1">
...
export default {
  ...
  computed: {
    ...
    subtitle() {
      return this.$t('Units from database');
    }
  },
  ...
  methods: {
    async onItem(id) {
      try {
        ...
        this.status = this.IDItem
          ? this.$t('Edit current record')
          : this.$t('Create new record');
        this.items = await this.$store.dispatch('api/unit/findAll');
        if (this.IDItem) {
    ...
    async onSave() {
      if (this.$refs.form.validate()) {
        try {
          if (this.IDItem) {
            await this.$store.dispatch('api/unit/updateOne', {
              id: this.IDItem,
              data: {
                ...this.item
              }
            ...
            await this.$store.dispatch('api/unit/createOne', {
              ...this.item
            });
            await this.onUpdate();
    ...
    async onDelete() {
      try {
        if (this.IDItem) {
          await this.$store.dispatch('api/unit/removeOne', {
            id: this.IDItem
          });
          await this.onUpdate();
    ...
    async onUpdate() {
      try {
        this.onClean();
        this.items = await this.$store.dispatch('api/unit/findAll');
        this.$toast.success(this.$t('Records is updated'));
      } catch (err) {
        colsole.error(err);
        this.$toast.error(err.message);
      }
